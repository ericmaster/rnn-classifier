...
class Lightning_CRNN(pl.LightningModule):
    def __init__(self, input_dim, num_classes, lr, dropout_rate=0.3, weight_decay=1e-4):
        ...
        self.valid_f1 = torchmetrics.F1Score(task='multiclass', num_classes=num_classes, average='macro')
        
        # Inicializar mixup augmentation
    ...
    def _shared_step(self, batch):
        ...
        # Calcular pérdida (considerando mixup)
        # loss = mixup_criterion(F.cross_entropy, logits, y)
        loss = F.cross_entropy(logits, y)  # Usar pérdida normal (recibe logits)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Actualizar métricas correctamente
        self.train_acc.update(preds, y)
        self.train_f1.update(preds, y)
        
        # Log con sync_dist para entornos distribuidos
        ...
        self.log("train_f1", self.train_f1, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("valid_loss", loss, prog_bar=False, sync_dist=True)
        self.log("valid_acc", self.valid_acc, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True)
        self.log("valid_f1", self.valid_f1, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
